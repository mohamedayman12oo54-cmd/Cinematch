...
print(f"TF-IDF matrix: {tfidf_matrix.shape[0]} titles x {tfidf_matrix.shape[1]} terms")
...
def get_recommendations(title: str, n: int = 10) -> pd.DataFrame:
    ...
    if title_lower not in indices:
        ...
        if not matches:
            print(f"  x  '{title}' not found. Try a different spelling.")
            return pd.DataFrame()
    ...
    rec_indices = [i for i,  in simscores]
    rec_scores  = [round(s, 4) for , s in simscores]
