...
test('the guest popular section gets poster_url attached per item, null when TMDB has no match', function () {
    ...
    $this->mock(TmdbMappingService::class, function ($mock) {
        $mock->shouldReceive('getPostersForTitles')
            ...
            ->andReturn(['Breaking Bad' => 'https://image.tmdb.org/t/p/w500/bb.jpg']);
    });
...
test('getPostersForTitles is called exactly once for the whole feed, batching every section together', function () {
    ...
    $this->mock(TmdbMappingService::class, function ($mock) {
        $mock->shouldReceive('getPostersForTitles')
            ...
            ->andReturn([
                'Better Call Saul' => 'https://img/bcs.jpg',
                'Zodiac' => 'https://img/zodiac.jpg',
                'Narcos' => 'https://img/narcos.jpg',
            ]);
    });
...
test('an item with a type label TMDB cannot map is excluded from the poster batch and gets a null poster', function () {
    $this->mock(MLClientService::class, function ($mock) {
        $mock->shouldReceive('getManyTitleDetails')->once()->andReturn([
            'Breaking Bad' => homeTmdbTitleDetail('Breaking Bad', 'TV Show', 2008),
    ...
    $this->mock(TmdbMappingService::class, function ($mock) {
        $mock->shouldReceive('getPostersForTitles')
            ...
            ->andReturn(['Breaking Bad' => 'https://img/bb.jpg']);
    });
...
test('getPostersForTitles is never called when every section is empty', function () {
    $this->mock(MLClientService::class, function ($mock) {
        $mock->shouldReceive('getManyTitleDetails')->once()->andReturn([]);
    });
    $this->mock(TmdbMappingService::class, function ($mock) {
        $mock->shouldReceive('getPostersForTitles')->never();
    });
